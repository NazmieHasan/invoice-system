<x-app-layout>
    <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-gray-100 dark:bg-gray-900">
        <h1 class="text-dark text-lg font-semibold">Invoice number {{ $invoice->invoice_number }}</h1>
        <div class="w-full sm:max-w-xl mt-6 px-6 py-4 bg-white dark:bg-gray-800 shadow-md overflow-hidden sm:rounded-lg">
            <div class="text-dark mb-4">
                <p>Customer Name: {{ $invoice->customer_name }}</p>
				<p>Customer Email: {{ $invoice->customer_email }}</p>
				<hr />
				<p class="font-semibold">All Line Items In Invoice</p>
				<hr />
				@foreach ($allLineItemInInvoice as $lineItem)
					<p>{{ $lineItem }}$</p>
				@endforeach
				<hr />
				<p class="font-semibold">Total Amount: {{ $invoice->total_amount }}$</p>
				<hr />
